...
public class TransportClusterHAHealthAction extends TransportMasterNodeReadAction<ClusterHealthRequest, ClusterHAHealthResponse> {
    ...
    protected void masterOperation(
        ...
        if (request.waitForEvents() != null) {
            ...
            executeHealth(
                    ...
                    listener,
                    waitCount,
                    clusterState -> listener.onResponse(getResponse(request, clusterState, waitCount, TransportClusterHAHealthAction.TimeoutState.OK))
            );
        }
    }
                    ...
                    new ClusterStateUpdateTask(request.waitForEvents()) {
                        ...
                        public void onFailure(String source, Exception e) {
                            if (e instanceof ProcessClusterEventTimeoutException) {
                                listener.onResponse(getResponse(request, clusterService.state(), waitCount, TransportClusterHAHealthAction.TimeoutState.TIMED_OUT));
                            } else {
                                logger.error(() -> new ParameterizedMessage("unexpected failure during [{}]", source), e);
                                listener.onFailure(e);
                            }
    ...
    private void executeHealth(
        ...
        if (request.timeout().millis() == 0) {
            listener.onResponse(getResponse(request, currentState, waitCount, TransportClusterHAHealthAction.TimeoutState.ZERO_TIMEOUT));
            return;
        }
        ...
        if (validationPredicate.test(currentState)) {
            listener.onResponse(getResponse(request, currentState, waitCount, TransportClusterHAHealthAction.TimeoutState.OK));
        } else {
            ...
            final ClusterStateObserver.Listener stateListener = new ClusterStateObserver.Listener() {
                ...
                public void onTimeout(TimeValue timeout) {
                    listener.onResponse(getResponse(request, observer.setAndGetObservedState(), waitCount, TransportClusterHAHealthAction.TimeoutState.TIMED_OUT));
                }
            };
    ...
    private boolean validateRequest(final ClusterHealthRequest request, ClusterState clusterState, final int waitCount) {
        ClusterHAHealthResponse response = clusterHealth(
                request,
                clusterState,
    ...
    private ClusterHAHealthResponse getResponse(
            final ClusterHealthRequest request,
            ClusterState clusterState,
            final int waitFor,
            TransportClusterHAHealthAction.TimeoutState timeoutState
    ) {
        ClusterHAHealthResponse response = clusterHealth(
                request,
                clusterState,
        ...
        assert valid || (timeoutState != TransportClusterHAHealthAction.TimeoutState.OK);
        ...
        response.setTimedOut(valid == false || timeoutState == TransportClusterHAHealthAction.TimeoutState.TIMED_OUT);
        return response;
    }

    static int prepareResponse(
            final ClusterHealthRequest request,
            final ClusterHAHealthResponse response,
    ...
    private ClusterHAHealthResponse clusterHealth(
        ...
        try {
            ...
            ClusterHAHealthResponse response = new ClusterHAHealthResponse(
        ...
        return new ClusterHAHealthResponse(
                clusterState.getClusterName().value(),
                concreteIndices,
                clusterState,
                numberOfPendingTasks,
